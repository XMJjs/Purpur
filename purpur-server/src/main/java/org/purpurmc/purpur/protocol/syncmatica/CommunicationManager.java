...
import org.purpurmc.purpur.protocol.syncmatica.exchange.DownloadExchange;
import org.purpurmc.purpur.protocol.syncmatica.exchange.Exchange;
import org.purpurmc.purpur.protocol.syncmatica.exchange.ExchangeTarget;
import org.purpurmc.purpur.protocol.syncmatica.exchange.ModifyExchangeServer;
import org.purpurmc.purpur.protocol.syncmatica.exchange.UploadExchange;
import org.purpurmc.purpur.protocol.syncmatica.exchange.VersionHandshakeServer;
...
@PurpurProtocol.Register(namespace = "syncmatica")
public class CommunicationManager implements PurpurProtocol {
    ...
    protected static void handle(ExchangeTarget source, @NotNull Identifier id, FriendlyByteBuf packetBuf) {
        if (id.equals(PacketType.REQUEST_LITEMATIC.identifier)) {
            ...
            try {
                ...
                PurpurLogger.LOGGER.error(e.toString());
                return;
            }
        ...
        if (id.equals(PacketType.MODIFY_REQUEST.identifier)) {
            ...
            if (modifier.getPlacement() == null) {
                PurpurLogger.LOGGER.warn("Could not find placement for modify request {}", placementId);
                return;
            }
    ...
    public boolean isActive() {
        return PurpurConfig.protocol.syncmatica.enable;
    }
}
