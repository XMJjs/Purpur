...
@PurpurProtocol.Register(namespace = "carpet")
public class CarpetServerProtocol implements PurpurProtocol {
    ...
    private static void handleHello(@NotNull ServerPlayer player, @NotNull CarpetServerProtocol.CarpetPayload payload) {
        ...
        PurpurLogger.LOGGER.info("Player {} joined with carpet {}", player.getScoreboardName(), payload.nbt.getString(HELLO).orElse("Unknown"));
    ...
    public boolean isActive() {
        return PurpurConfig.protocol.leavesCarpetSupport;
    }
    ...
    public record CarpetPayload(CompoundTag nbt) implements PurpurCustomPayload {
        @ID
